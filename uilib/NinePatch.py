...
@description: 修改自https://github.com/Roninsc2/NinePatchQt
'''
...
class ExceptionIncorrectWidthAndHeight(_Exception):
    ...
    def __str__(self):
        return "Input incorrect width width  height. Minimum width = :{imgW} . Minimum height = :{imgH}".format(imgW = self.imgW, imgH = self.imgH)
...
class NinePatch:
    ...
    def Draw(self, painter, x, y):
        # 绘制
        # print("TNinePatch.Draw  :  x:%d y:%d" % (x, y))
